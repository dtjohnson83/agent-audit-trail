...
// ============================================================
// Policy engine + log store (inline)
// ============================================================
const logs = [];
...
function evaluatePolicy(toolName, toolAction, parameters, dataFields) {
  ...
  if (vals.some(v => v > 1000)) {
    violations.push({ rule_id: "financial-threshold", rule_name: "High-Value Financial Action", severity: "high", action_taken: "flag", details: "Rule triggered." });
  }
  return violations;
}
...
function getSummary() {
  ...
  let violations = 0;
  for (const l of logs) { byAgent[l.agent_name] = (byAgent[l.agent_name]||0)+1; byTool[l.tool_name] = (byTool[l.tool_name]||0)+1; if(l.policy_violations.length>0) violations++; }
...
function handleMcp(body) {
  ...
  if (method === "tools/call") {
    ...
    if (name === "log_action") {
      const dataFields = typeof args.data_fields_accessed === "string" ? args.data_fields_accessed.split(",").map(s=>s.trim()).filter(Boolean) : Array.isArray(args.data_fields_accessed) ? args.data_fields_accessed : [];
      const entry = { agent_id: args.agent_id||"vercel-agent", agent_name: args.agent_name||"unknown", tool_name: args.tool_name||"unknown", tool_action: args.tool_action||"", parameters: args.parameters||{}, response_summary: "logged", response_status: "success", data_fields_accessed: dataFields, execution_duration_ms: args.execution_duration_ms||0, token_cost_estimate: args.token_cost_estimate??null, policy_violations: [], metadata: args.metadata||{} };
...
<html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>AI Agent Audit</title><meta name="description" content="Compliance infrastructure for agentic AI."/><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet"/>
...
<div class="nl"><a href="https://github.com/danzusholdings/agent-audit-trail" target="_blank" rel="noopener">GitHub</a><a href="#waitlist" class="cta">Get Early Access</a></div></nav>
<section class="hero">
...
<section class="sec"><div class="prob"><h2>The problem</h2>
...
<p class="pn">Without an audit trail, the answer is no. Agent Audit Trail gives you the paper trail so you can say yes.</p></div></section>
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === "/api/mcp") {
      if (req.method === "GET") {
        res.writeHead(200, { "Content-Type": "text/event-stream", "Cache-Control": "no-cache", "Connection": "keep-alive", "MCP-Session-Id": url.searchParams.get("sessionId") || "stateless" });
