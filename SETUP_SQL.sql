-- Agent Audit Trail - Supabase Schema
-- Run this in your Supabase SQL Editor: https://supabase.com/dashboard/project/gdmeyoikofpkulyqwzgi/sql
...
-- ============================================================
-- Audit logs (immutable via hash chain)
-- ============================================================
CREATE TABLE audit_logs (
  ...
  -- Immutability: no UPDATE or DELETE allowed via policies
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Allow anon read on logs and agents
...
CREATE POLICY anon_read_alert_configs ON alert_configs FOR SELECT USING (true);
CREATE POLICY anon_read_audit_logs ON audit_logs FOR SELECT USING (true);

-- Allow anon insert on all tables
...
CREATE POLICY anon_insert_alert_configs ON alert_configs FOR INSERT WITH CHECK (true);
CREATE POLICY anon_insert_audit_logs ON audit_logs FOR INSERT WITH CHECK (true);

-- Allow anon update on agents (for last_active, total_actions)
CREATE POLICY anon_update_agents ON agents FOR UPDATE USING (true);

-- Block DELETE on audit_logs (immutability)
CREATE POLICY no_delete_audit_logs ON audit_logs FOR DELETE USING (false);

-- Block UPDATE on audit_logs (immutability - only insert allowed)
CREATE POLICY no_update_audit_logs ON audit_logs FOR UPDATE USING (false);

-- ============================================================
-- Default policy rules
-- ============================================================
INSERT INTO policy_rules (rule_id, name, description, condition_type, condition_operator, condition_value, action, severity) VALUES
('pii-field-access', 'PII Field Access', 'Flags access to PII fields: SSN, passport, drivers license, dob, email, phone, address', 'data_field_match', 'contains', 'ssn,passport,drivers_license,dob,date_of_birth,email,phone,address,credit_card', 'flag', 'high'),
('destructive-db', 'Destructive Database Operation', 'Flags DELETE, DROP, TRUNCATE, or DESTROY operations', 'tool_match', 'contains', 'delete,drop,truncate,destroy,remove', 'flag', 'critical'),
('high-value-financial', 'High-Value Financial Action', 'Flags financial actions with amounts over $1,000', 'parameter_match', 'greater_than', '1000', 'flag', 'high');

-- ============================================================
-- Helper function: upsert agent on first log
-- ============================================================
...
SELECT 'Agent Audit Trail database ready!' AS status;
